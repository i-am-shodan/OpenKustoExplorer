...
public sealed class KustoChart : Control
{
    ...
    private static Rect GetColumnRect(
        ...
        int seriesCount)
    {
        double x = MapX(plotBounds, visualization, point.X);
        ...
        double groupWidth = Math.Max(8, plotBounds.Width / categoryCount * 0.7);
        ...
        int seriesCount)
    {
        ...
        double y = MapXAsY(plotBounds, visualization, point.X);
        double categoryCount = Math.Max(1, visualization.VisibleSeries.SelectMany(series => series.Points).Select(item => item.X).Distinct().Count());
        double groupHeight = Math.Max(8, plotBounds.Height / categoryCount * 0.7);
        ...
        double value)
    {
        double position = Normalize(
            ...
            visualization.XMaximum,
            visualization.XAxisLogarithmic);
        return plotBounds.Top + (position * plotBounds.Height);
    }
        ...
        Dictionary<double, double> totals)
    {
        ...
        double categoryCount = Math.Max(1, visualization.VisibleSeries.SelectMany(series => series.Points).Select(item => item.X).Distinct().Count());
        double width = Math.Max(4, plotBounds.Width / categoryCount * 0.65);
        ...
        Dictionary<double, double> totals)
    {
        ...
        double y = MapXAsY(plotBounds, visualization, point.X);
        double categoryCount = Math.Max(1, visualization.VisibleSeries.SelectMany(series => series.Points).Select(item => item.X).Distinct().Count());
        double height = Math.Max(4, plotBounds.Height / categoryCount * 0.65);
