...
internal sealed class KustoEditorController : IDisposable
{
        ...
        KustoLanguageAnalysis analysis)
    {
        ...
        isCurrentDocument &= viewModel.ActiveSchemaRevision == schemaRevision;
        ...
        if (isCurrentDocument && !isDisposed)
        {
            using WorkbenchPerformanceScope applyMeasurement = new(
                ...
                analysis.Classifications.Count);
    ...
    private async Task ShowCompletionAfterDelayAsync(
        ...
        CancellationToken cancellationToken)
    {
        long requestOperationId = performanceSink.StartOperation(
            "editor.completion.paint",
            textSnapshot.Length);
        try
        {
            await Task.Delay(delay, cancellationToken).ConfigureAwait(false);
            ...
            await performanceSink.CompleteOperationAfterRenderAsync(
                requestOperationId,
                completionWindow is null ? "empty" : "shown");
        }
        catch (OperationCanceledException)
        {
            performanceSink.CompleteOperation(requestOperationId, "canceled");

            // A newer keystroke or document superseded the pending completion request.
        }
        catch (Exception exception)
        {
            performanceSink.CompleteOperation(requestOperationId, "failed");
            await Dispatcher.UIThread.InvokeAsync(
                () => viewModel.ReportAnalysisFailure(exception.Message),
                DispatcherPriority.Background,
                CancellationToken.None);
        ...
        KustoLanguageAnalysis analysis)
    {
        bool canShow = !isDisposed
            ...
            && viewModel.SelectedDocument?.Id == documentId
            && viewModel.ActiveSchemaRevision == schemaRevision;
