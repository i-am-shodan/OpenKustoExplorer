...
/// <summary>
/// Hosts the primary Open Kusto Explorer workbench.
/// </summary>
public sealed partial class WorkbenchView : UserControl, IDisposable
{
    private const double ActivityRailWidth = 48;
    private const double CompactLayoutBreakpoint = 1080;
    private const double CopilotPanelWidth = 440;
    private const double MobileLayoutBreakpoint = 620;
    ...
    private static readonly TimeSpan ResultTextCommitDelay = TimeSpan.FromMilliseconds(150);
    ...
    private readonly DispatcherTimer resultColumnFilterCommitTimer;
    private readonly DispatcherTimer resultSearchCommitTimer;
    private Button? appearanceButton;
    private AppearanceSettings? appearanceSettings;
    private Button? automationButton;
    private IWorkbenchAutomationNotificationDispatcher? automationNotificationDispatcher;
    ...
    private StackPanel? headerBrand;
    private Border? headerDivider;
    ...
    private Button? settingsLocalDataAction;
    private TextBlock? settingsLocalDataActionText;
    ...
    private KustoResultColumnViewModel? pendingResultColumnFilter;
    private long pendingResultColumnFilterOperationId;
    private string? pendingResultColumnFilterText;
    private long pendingResultContainersOperationId;
    private long pendingResultFirstRowOperationId;
    private EventHandler? pendingResultRowsLayoutHandler;
    private EventHandler? pendingWorkspaceLayoutHandler;
    private string? pendingWorkspaceName;
    private long pendingWorkspaceOperationId;
    private SplitView? pertinentValuesSplitView;
    private IWorkbenchPerformanceSink performanceSink = NullWorkbenchPerformanceSink.Instance;
    private Action? storageManagementAction;
    ...
    private Border? tabSearchContainer;
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="WorkbenchView"/> class for compiled XAML and design tools.
    /// </summary>
    public WorkbenchView()
    {
        resultSearchCommitTimer = new DispatcherTimer(
            ResultTextCommitDelay,
            DispatcherPriority.Background,
            OnResultSearchCommitTimerTick);
        resultColumnFilterCommitTimer = new DispatcherTimer(
            ResultTextCommitDelay,
            DispatcherPriority.Background,
            OnResultColumnFilterCommitTimerTick);
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="WorkbenchView"/> class.
    /// </summary>
    ...
    /// <param name="notificationDispatcher">The required host notification dispatcher.</param>
    /// <param name="performanceSink">The optional detailed workbench performance sink.</param>
    /// <param name="storageManagementAction">The optional host browser-storage management action.</param>
    ...
    public WorkbenchView(
        ...
        IWorkbenchAutomationNotificationDispatcher notificationDispatcher,
        IWorkbenchPerformanceSink? performanceSink = null,
        Action? storageManagementAction = null)
        : this()
    {
        ...
        if (viewModel.HostCapabilities.StorageManagementMode == WorkbenchStorageManagementMode.BrowserDialog
            && storageManagementAction is null)
        {
            throw new ArgumentNullException(
                nameof(storageManagementAction),
                "Browser storage management requires a host action.");
        }

        this.performanceSink = performanceSink ?? NullWorkbenchPerformanceSink.Instance;
        this.storageManagementAction = storageManagementAction;
        DataContext = viewModel;
        InitializeInteractiveControls();
        if (this.performanceSink.IsEnabled)
        {
            viewModel.ResultRows.CollectionChanged += OnResultRowsCollectionChanged;
        }

        if (storageManagementAction is not null)
        {
            AutomationProperties.SetName(settingsLocalDataAction!, "Manage browser storage");
            settingsLocalDataActionText!.Text = "Manage storage";
        }

        automationNotificationDispatcher = notificationDispatcher;
        viewModel.AutomationNotificationRequested += OnAutomationNotificationRequested;
        this.appearanceSettings = appearanceSettings;
        viewModel.ApplyCopilotDefaults(CreateCopilotDefaults(appearanceSettings, viewModel.HostCapabilities));
        ...
        editorController = new KustoEditorController(
            queryEditor!,
            viewModel,
            appearanceSettings,
    ...
    /// <summary>
    /// Occurs when the user invokes an available host-managed update.
    /// </summary>
    internal event EventHandler? OpenUpdateRequested;
    ...
    public void Dispose()
    {
        if (!isDisposed)
        {
            ...
            CancelPendingResultTextCommits("disposed");
            resultSearchCommitTimer.Tick -= OnResultSearchCommitTimerTick;
            resultColumnFilterCommitTimer.Tick -= OnResultColumnFilterCommitTimerTick;
            ...
            if (automationNotificationDispatcher is IDisposable disposableDispatcher)
            {
                disposableDispatcher.Dispose();
            }

            automationNotificationDispatcher = null;
            OpenUpdateRequested = null;
            storageManagementAction = null;
            CancelPendingResultRowsPaint("disposed");
            CancelPendingWorkspaceOperation("disposed");
    ...
    /// <summary>
    /// Focuses one constrained control used by the Browser performance fixture.
    /// </summary>
    /// <param name="target">The fixture target name.</param>
    /// <returns><see langword="true"/> when the target exists and was focused.</returns>
    ...
    /// <returns><see langword="true"/> when the document exists and the editor is visible.</returns>
    internal bool FocusPerformanceDocument(Guid documentId)
    {
        if (DataContext is not MainWindowViewModel viewModel
            || queryEditor is null
            || !queryEditor.IsEffectivelyVisible
            || viewModel.Documents.FirstOrDefault(document => document.Id == documentId)
                is not KustoDocumentViewModel document)
        {
            return false;
        }
    ...
    /// <summary>
    /// Reveals the host-managed update action for an available release.
    /// </summary>
    /// <param name="tagName">The release tag shown in accessible text.</param>
    internal void ShowAvailableUpdate(string tagName)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(tagName);
        AutomationProperties.SetName(updateButton!, $"Update to {tagName}");
        ToolTip.SetTip(updateButton!, $"{tagName} is available. Open the official GitHub release.");
    ...
    private static KustoCopilotDefaults CreateCopilotDefaults(
        AppearanceSettings settings,
        WorkbenchHostCapabilities capabilities)
    {
        return new KustoCopilotDefaults(
            ...
            capabilities.SupportsMcp && settings.CopilotEnableMicrosoftLearnMcpByDefault,
            capabilities.SupportsMcp && settings.CopilotEnableAzureMcpByDefault,
    ...
    private static bool IsWorkspaceActive(MainWindowViewModel viewModel, string workspaceName)
    {
        return workspaceName switch
        {
            "query" => viewModel.IsQueryWorkbenchView,
            "dashboard" => viewModel.IsDashboardView,
            "automation" => viewModel.IsAutomationView,
            "sessions" => viewModel.IsSessionsView,
            "graph" => viewModel.IsGraphView,
            _ => false,
        };
    }

    private static async Task ObservePerformanceTaskAsync(Task task)
    {
        try
        {
            await task;
        ...
        catch (Exception exception)
        {
            System.Diagnostics.Trace.TraceError(
                "Unexpected workbench performance instrumentation failure: {0}",
                exception);
        }
    }
    ...
    private static T FindRequiredDescendant<T>(Control root, string controlName)
        where T : Control
    {
        T control = root.GetVisualDescendants()
            .OfType<T>()
            .FirstOrDefault(candidate => string.Equals(candidate.Name, controlName, StringComparison.Ordinal))
    ...
    private void OnAppearancePropertyChanged(object? sender, PropertyChangedEventArgs eventArguments)
    {
            ...
            && DataContext is MainWindowViewModel viewModel)
        {
            ...
            {
                viewModel.ApplyCopilotDefaults(CreateCopilotDefaults(settings, viewModel.HostCapabilities));
            }
            else if (viewModel.HostCapabilities.IsAIProviderUserConfigured
                && IsAIProviderProperty(eventArguments.PropertyName))
            {
    ...
    {
        MarkWorkspaceRequest("dashboard");
        if (DataContext is MainWindowViewModel viewModel)
        {
        ...
        Dispatcher.UIThread.Post(
            () =>
            {
                if (!isDisposed && DataContext is MainWindowViewModel { IsDashboardView: true } viewModel)
                {
                    _ = RunAutomationTickAsync(viewModel);
                }
            },
    ...
    {
        MarkWorkspaceRequest("automation");
        if (DataContext is MainWindowViewModel viewModel)
        {
    ...
    {
        MarkWorkspaceRequest("sessions");
        Task? activationTask = null;
        if (DataContext is MainWindowViewModel viewModel)
        {
            activationTask = viewModel.ShowSessionsCommand.ExecuteAsync(null);
        }
    ...
    private void OnGraphLayoutLoaded(object? sender, RoutedEventArgs eventArguments)
    {
        if (sender is Control graphLayout)
        {
            graphCanvas = FindRequiredDescendant<KustoGraphControl>(graphLayout, "GraphCanvas");
            graphViewportToolbar = FindRequiredDescendant<Border>(graphLayout, "GraphViewportToolbar");
            performanceSink.Mark("graph.layout.materialized");
        }
    }
    ...
    private void OnAutomationNotificationEditorLoaded(object? sender, RoutedEventArgs eventArguments)
    {
        _ = eventArguments;
        if (sender is not Control root || automationNotificationDispatcher is null)
        {
            return;
        }
        ...
        WorkbenchHostCapabilities capabilities = viewModel.HostCapabilities;
        ToggleSwitch toastAction = FindRequiredDescendant<ToggleSwitch>(root, "AutomationToastAction");
        ToggleSwitch emailAction = FindRequiredDescendant<ToggleSwitch>(root, "AutomationEmailAction");
        ToggleSwitch applicationAction = FindRequiredDescendant<ToggleSwitch>(root, "AutomationApplicationAction");
        ToggleSwitch webhookAction = FindRequiredDescendant<ToggleSwitch>(root, "AutomationWebhookAction");
        toastAction.Content = capabilities.ToastNotificationName;
        toastAction.IsVisible = capabilities.SupportsToastNotifications;
        AutomationProperties.SetName(
            toastAction,
            $"{capabilities.ToastNotificationName} automation notifications");
        emailAction.IsVisible = capabilities.SupportsEmailNotifications;
        applicationAction.IsVisible = capabilities.SupportsApplicationLaunchNotifications;
        webhookAction.IsVisible = capabilities.SupportsWebhookNotifications;
        if (!capabilities.SupportsEmailNotifications)
        {
            viewModel.AutomationNotifications.EmailEnabled = false;
            FindRequiredDescendant<Grid>(root, "AutomationEmailSettings").IsVisible = false;
        }

        if (!capabilities.SupportsApplicationLaunchNotifications)
        {
            viewModel.AutomationNotifications.RunApplicationEnabled = false;
            FindRequiredDescendant<Grid>(root, "AutomationApplicationSettings").IsVisible = false;
        }

        if (!capabilities.SupportsWebhookNotifications)
        {
            viewModel.AutomationNotifications.WebhookEnabled = false;
            FindRequiredDescendant<StackPanel>(root, "AutomationWebhookSettings").IsVisible = false;
        }
    }
    ...
    private void OnResultRowsCollectionChanged(
        object? sender,
        NotifyCollectionChangedEventArgs eventArguments)
    {
        ...
        CancelPendingResultRowsPaint("superseded");
        ...
        layoutUpdatedHandler = (_, _) =>
        {
            ...
            KustoResultRowViewModel[] realizedRows = resultsList
            ...
            pendingResultRowsLayoutHandler = null;
            int firstRowIndex = realizedRows.Min(row => row.RowIndex);
            long firstRowOperationId = pendingResultFirstRowOperationId;
            long containersOperationId = pendingResultContainersOperationId;
            pendingResultFirstRowOperationId = 0;
            pendingResultContainersOperationId = 0;
            _ = ObservePerformanceTaskAsync(performanceSink.CompleteOperationAfterRenderAsync(
                firstRowOperationId,
                $"row:{firstRowIndex}"));
            _ = ObservePerformanceTaskAsync(performanceSink.CompleteOperationAfterRenderAsync(
                containersOperationId,
                $"realized:{realizedRows.Length}"));
        };
        pendingResultRowsLayoutHandler = layoutUpdatedHandler;
        LayoutUpdated += layoutUpdatedHandler;
    }

    private void CancelPendingResultRowsPaint(string outcome)
    {
        if (pendingResultRowsLayoutHandler is not null)
        {
            LayoutUpdated -= pendingResultRowsLayoutHandler;
            pendingResultRowsLayoutHandler = null;
        }

        if (pendingResultFirstRowOperationId > 0)
        {
            performanceSink.CompleteOperation(pendingResultFirstRowOperationId, outcome);
            pendingResultFirstRowOperationId = 0;
        }

        if (pendingResultContainersOperationId > 0)
        {
            performanceSink.CompleteOperation(pendingResultContainersOperationId, outcome);
            pendingResultContainersOperationId = 0;
        }
    }

    private void OnResultSearchTextChanged(object? sender, TextChangedEventArgs eventArguments)
    {
        _ = eventArguments;
        if (sender is not TextBox input || DataContext is not MainWindowViewModel viewModel)
        {
            return;
        }

        string text = input.Text ?? string.Empty;
        if (string.Equals(text, viewModel.ResultSearchText, StringComparison.Ordinal))
        {
            CancelPendingResultSearch("synchronized");
            return;
        }

        CancelPendingResultSearch("superseded");
        pendingResultSearchText = text;
        pendingResultSearchOperationId = performanceSink.StartOperation(
            "results.search.paint",
            viewModel.ResultRows.Count);
        resultSearchCommitTimer.Start();
    }

    private void OnResultSearchLostFocus(object? sender, RoutedEventArgs eventArguments)
    {
        _ = sender;
        _ = eventArguments;
        CommitPendingResultSearch();
    }

    private void OnResultSearchKeyDown(object? sender, KeyEventArgs eventArguments)
    {
        if (eventArguments.Key == Key.Enter)
        {
            ...
            && DataContext is MainWindowViewModel viewModel)
        {
            CancelPendingResultSearch("canceled");
            input.Text = viewModel.ResultSearchText;
            input.CaretIndex = input.Text?.Length ?? 0;
            eventArguments.Handled = true;
        }
    }

    private void OnResultSearchCommitTimerTick(object? sender, EventArgs eventArguments)
    {
        _ = sender;
        _ = eventArguments;
        CommitPendingResultSearch();
    ...
    private void OnResultColumnFilterTextChanged(object? sender, TextChangedEventArgs eventArguments)
    {
        _ = eventArguments;
        if (sender is not TextBox { DataContext: KustoResultColumnViewModel column } input)
        {
            return;
        }

        string text = input.Text ?? string.Empty;
        if (string.Equals(text, column.FilterText, StringComparison.Ordinal))
        {
            if (ReferenceEquals(pendingResultColumnFilter, column))
            {
                CancelPendingResultColumnFilter("synchronized");
            }

            return;
        }

        if (pendingResultColumnFilter is not null
            && !ReferenceEquals(pendingResultColumnFilter, column))
        {
            CommitPendingResultColumnFilter();
        }

        CancelPendingResultColumnFilter("superseded");
        pendingResultColumnFilter = column;
        pendingResultColumnFilterText = text;
        pendingResultColumnFilterOperationId = performanceSink.StartOperation(
            "results.filter.paint",
            DataContext is MainWindowViewModel viewModel ? viewModel.ResultRows.Count : 0);
        resultColumnFilterCommitTimer.Start();
    }

    private void OnResultColumnFilterLostFocus(object? sender, RoutedEventArgs eventArguments)
    {
        _ = sender;
        _ = eventArguments;
        CommitPendingResultColumnFilter();
    ...
    private void OnResultColumnFilterKeyDown(object? sender, KeyEventArgs eventArguments)
    {
        if (eventArguments.Key == Key.Enter)
        {
            CommitPendingResultColumnFilter();
            ...
            && sender is TextBox { DataContext: KustoResultColumnViewModel column } input)
        {
            CancelPendingResultColumnFilter("canceled");
            input.Text = column.FilterText;
            input.CaretIndex = input.Text?.Length ?? 0;
            eventArguments.Handled = true;
        }
    }

    private void OnResultColumnFilterCommitTimerTick(object? sender, EventArgs eventArguments)
    {
        _ = sender;
        _ = eventArguments;
        CommitPendingResultColumnFilter();
    ...
    private void CommitPendingResultSearch()
    {
        resultSearchCommitTimer.Stop();
        string? text = pendingResultSearchText;
        long operationId = pendingResultSearchOperationId;
        pendingResultSearchText = null;
        pendingResultSearchOperationId = 0;
        if (text is not null && DataContext is MainWindowViewModel viewModel)
        {
            viewModel.ResultSearchText = text;
        }

        if (operationId > 0)
        {
            _ = ObservePerformanceTaskAsync(
                performanceSink.CompleteOperationAfterRenderAsync(operationId));
        }
    }

    private void CommitPendingResultColumnFilter()
    {
        resultColumnFilterCommitTimer.Stop();
        KustoResultColumnViewModel? column = pendingResultColumnFilter;
        string? text = pendingResultColumnFilterText;
        long operationId = pendingResultColumnFilterOperationId;
        pendingResultColumnFilter = null;
        pendingResultColumnFilterText = null;
        pendingResultColumnFilterOperationId = 0;
        if (column is not null && text is not null)
        {
    ...
    private void CancelPendingResultTextCommits(string outcome)
    {
        CancelPendingResultSearch(outcome);
        CancelPendingResultColumnFilter(outcome);
    }

    private void CancelPendingResultSearch(string outcome)
    {
        resultSearchCommitTimer.Stop();
        pendingResultSearchText = null;
        if (pendingResultSearchOperationId > 0)
        {
            performanceSink.CompleteOperation(pendingResultSearchOperationId, outcome);
            pendingResultSearchOperationId = 0;
        }
    }

    private void CancelPendingResultColumnFilter(string outcome)
    {
        resultColumnFilterCommitTimer.Stop();
        pendingResultColumnFilter = null;
        pendingResultColumnFilterText = null;
        if (pendingResultColumnFilterOperationId > 0)
        {
            performanceSink.CompleteOperation(pendingResultColumnFilterOperationId, outcome);
            pendingResultColumnFilterOperationId = 0;
        }
    }
    ...
    {
        if (appearanceSettings is not null
            && DataContext is MainWindowViewModel { HostCapabilities.SupportsMcp: true }
    ...
    private void OnSettingsAIProviderChanged(object? sender, SelectionChangedEventArgs eventArguments)
    {
        ...
        if (!suppressSettingsAIProviderChange
            ...
            && DataContext is MainWindowViewModel { HostCapabilities.IsAIProviderUserConfigured: true }
    ...
    private void OnSettingsAIProviderTextLostFocus(object? sender, RoutedEventArgs eventArguments)
    {
        _ = sender;
        _ = eventArguments;
        if (DataContext is MainWindowViewModel { HostCapabilities.IsAIProviderUserConfigured: true })
        {
    ...
    private async void OnOpenRecordingClick(object? sender, RoutedEventArgs eventArguments)
    {
        ...
        if (DataContext is MainWindowViewModel viewModel)
        {
            ...
            Dispatcher.UIThread.Post(() => this
                .GetVisualDescendants()
                .OfType<TextBox>()
                .FirstOrDefault(control => string.Equals(
                    control.Name,
                    "RecordingNameTextBox",
                    StringComparison.Ordinal))?
                .Focus());
        }
    }
    ...
    private void OnConditionalPresetClick(object? sender, RoutedEventArgs eventArguments)
    {
            ...
            && DataContext is MainWindowViewModel viewModel)
        {
            ...
            ColorPicker? colorPicker = this.GetVisualDescendants().OfType<ColorPicker>().FirstOrDefault();
            if (colorPicker is not null)
            {
                colorPicker.Color = Color.Parse(colorHex);
            }
        }
    }
    ...
    private void MarkWorkspaceRequest(string workspaceName)
    {
        if (!performanceSink.IsEnabled)
        {
            return;
        }

        CancelPendingWorkspaceOperation("superseded");
        pendingWorkspaceName = workspaceName;
        pendingWorkspaceOperationId = performanceSink.StartOperation(
            $"workspace.{workspaceName}.paint");
        EventHandler? layoutUpdatedHandler = null;
        layoutUpdatedHandler = (_, _) =>
        {
            if (DataContext is not MainWindowViewModel viewModel
                || !IsWorkspaceActive(viewModel, workspaceName)
                || pendingWorkspaceOperationId == 0
                || !string.Equals(pendingWorkspaceName, workspaceName, StringComparison.Ordinal))
            {
                return;
            }

            LayoutUpdated -= layoutUpdatedHandler;
            pendingWorkspaceLayoutHandler = null;
            long operationId = pendingWorkspaceOperationId;
            pendingWorkspaceOperationId = 0;
            pendingWorkspaceName = null;
            _ = ObservePerformanceTaskAsync(
                performanceSink.CompleteOperationAfterRenderAsync(operationId));
        };
        pendingWorkspaceLayoutHandler = layoutUpdatedHandler;
        LayoutUpdated += layoutUpdatedHandler;
    }

    private void CancelPendingWorkspaceOperation(string outcome)
    {
        if (pendingWorkspaceLayoutHandler is not null)
        {
            LayoutUpdated -= pendingWorkspaceLayoutHandler;
            pendingWorkspaceLayoutHandler = null;
        }

        if (pendingWorkspaceOperationId > 0)
        {
            performanceSink.CompleteOperation(pendingWorkspaceOperationId, outcome);
            pendingWorkspaceOperationId = 0;
            pendingWorkspaceName = null;
        }
    }
    ...
    private void InitializeInteractiveControls()
    {
        ...
        headerBrand = FindRequiredControl<StackPanel>("HeaderBrand");
        headerDivider = FindRequiredControl<Border>("HeaderDivider");
        ...
        settingsLocalDataAction = FindRequiredControl<Button>("SettingsLocalDataAction");
        settingsLocalDataActionText = FindRequiredControl<TextBlock>("SettingsLocalDataActionText");
        ...
        tabSearchContainer = FindRequiredControl<Border>("TabSearchContainer");
    ...
    private TopLevel GetRequiredTopLevel()
    {
        return TopLevel.GetTopLevel(this)
            ?? throw new InvalidOperationException("The workbench is not attached to a host.");
    }
    ...
    private async Task CopyTextAsync(string text, string successMessage)
    {
        try
        {
            ...
            IClipboard clipboard = GetRequiredTopLevel().Clipboard
    ...
    private async Task ExportResultsAsync(KustoResultExportFormat format)
    {
        if (DataContext is MainWindowViewModel viewModel)
        {
            try
            {
                ...
                IStorageFile? file = await GetRequiredTopLevel().StorageProvider.SaveFilePickerAsync(new FilePickerSaveOptions
                {
                    Title = $"Export {format}",
    ...
    private async Task ImportRecordedSessionAsync()
    {
        if (DataContext is MainWindowViewModel viewModel)
        {
            ...
            {
                FilePickerFileType fileType = CreateRecordedSessionArchiveFileType();
                IReadOnlyList<IStorageFile> files = await GetRequiredTopLevel().StorageProvider.OpenFilePickerAsync(
    ...
    {
        if (appearanceSettings is not null && DataContext is MainWindowViewModel viewModel)
        {
            WorkbenchHostCapabilities capabilities = viewModel.HostCapabilities;
            KustoAIProviderKind providerKind = capabilities.IsAIProviderUserConfigured
                ? appearanceSettings.ProviderKind
                : capabilities.ManagedAIProviderKind!.Value;
            suppressSettingsAIProviderChange = true;
            try
            {
                settingsAIProvider!.SelectedIndex = (int)providerKind;
            }
            finally
            {
            ...
            bool usesAzureOpenAI = providerKind == KustoAIProviderKind.AzureOpenAI;
            bool usesOpenAI = providerKind == KustoAIProviderKind.OpenAI;
            bool usesGitHubCopilot = providerKind == KustoAIProviderKind.GitHubCopilot;
            settingsAzureOpenAIOptions!.IsVisible = capabilities.IsAIProviderUserConfigured && usesAzureOpenAI;
            settingsOpenAIOptions!.IsVisible = capabilities.IsAIProviderUserConfigured && usesOpenAI;
            settingsCopilotDefaultModelRow!.IsVisible = capabilities.IsAIProviderUserConfigured
                && usesGitHubCopilot;
            settingsCopilotMicrosoftLearnMcpRow!.IsVisible = capabilities.SupportsMcp
                && usesGitHubCopilot;
            settingsCopilotAzureMcpRow!.IsVisible = capabilities.SupportsMcp
                && usesGitHubCopilot;
            ...
            if (settingsCopilotDefaultModel is not null)
            {
    ...
    private void UpdateResponsiveLayout(double width)
    {
        ...
        {
            bool useOverlay = ShouldOverlayCopilot(width);
            copilotPanel.Width = useOverlay
                ? Math.Min(CopilotPanelWidth, Math.Max(0, width - ActivityRailWidth))
                : CopilotPanelWidth;
    ...
    {
        bool useMobileLayout = width < MobileLayoutBreakpoint;
        if (headerBrand is not null)
        {
            headerBrand.IsVisible = !useMobileLayout;
        }

        if (headerDivider is not null)
        {
            headerDivider.IsVisible = !useMobileLayout;
        }

        if (tabSearchContainer is not null)
        {
            tabSearchContainer.Width = useMobileLayout ? 220 : 320;
        }
    }
